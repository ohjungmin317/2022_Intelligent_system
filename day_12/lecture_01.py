...
pp = ColumnTransformer([('s_mm', s_mm, mm_cols),('s_ss', s_ss, ss_cols),('s_rs', s_rs, rs_cols)],n_jobs=1)
...
param_grid = {'model__n_estimators' : [100, 50 , 10 ,200], 'model__max_depth': [None, 7, 10]}

cv = KFold(n_splits=15, shuffle=True, random_state=1)
grid = GridSearchCV(pipe, param_grid = param_grid, cv=cv, scoring='r2',verbose=3)
