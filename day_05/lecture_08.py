...
model = LogisticRegression(penalty='l2',
                           ...
                           class_weight={0:1000,1:0}
                           solver='lbfgs',max_iter =1000000,  
                           n_jobs=-1, random_state=5, verbose=3) 
# l1 - lasso /  l2 - Ridge /  elasticnet - l1과 l2를 적절하게 섞어놓음 // default = l2
# c가 alpha를 대체해주는 것이다. // [ alpha는 값이 커지면 커질 수록 제약이 커짐 ] [ c 값이 커지면 제약이 풀리고 값이 작아지면 제약이 커진다. ]
# class_weight('balanced') // 데이터의 개수가 많으면 많을 수록 더 유일함 
# 'sag' , 'saga' -> 경사 / n_jobs = -1 => 병렬처리 
